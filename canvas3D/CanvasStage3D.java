...
public class CanvasStage3D{
	// Canvasのプロパティ
	private Stage pstage;          // CanvasStage3Dの本質はStageである
	protected Pane root;           // Canvasを置くPane
	...
	/**
	 * CanvasStage3Dの本質はStageであり、Platform.startupによりアプリケーションを
	 * 実行する
	 */
	public CanvasStage3D(double width, double height, double cameraWidth, double cameraHeight,
		...
		Platform.startup(()->{
			...
			pstage.setTitle("3D Canvas");
			pstage.show();
		});
	}

	public CanvasStage3D(){
		// デフォルトに設定
	 ...
	 * 一つのShape3Dを描く
	 */
	synchronized private void drawShape(Shape3D shape){
